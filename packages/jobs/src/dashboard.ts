...
export function dashboardRoutes(app:FastifyInstance,{db,artifacts,reviews}:Services):void{
  app.get("/api/dashboard",async()=>({schemaVersion:SCHEMA_VERSION,limit:100,
    ...
    screening:db.prepare("SELECT d.id,d.job_id,d.decision,d.score,d.reasons_json,d.actor,d.created_at,j.title,j.company FROM screening_decisions d JOIN jobs j ON j.id=d.job_id ORDER BY d.created_at DESC, d.rowid DESC LIMIT 100").all(),
